...
def convert_safetensors_to_ckpt(safetensors_path: Path, ckpt_path: Path):
    if ckpt_path.exists():
...
def convert_directory(input_dir: str, output_dir: str | None):
    ...
    if not input_path.exists():
        print(f"[ERROR] 入力ディレクトリが存在しません: {input_path}")
        sys.exit(1)
    ...
    if total == 0:
        print("[INFO] safetensors が見つかりませんでした")
        return
    ...
    for idx, safepath in enumerate(safetensors_files, start=1):
        ...
        convert_safetensors_to_ckpt(safepath, ckpt_path)
